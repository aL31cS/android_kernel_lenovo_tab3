...
#define VOL_1000 1000000
	...
	if (On) {
		...
		if (pinSetIdx == 0 && currSensorName && ((0 == strcmp(currSensorName, SENSOR_DRVNAME_HI545_MIPI_RAW))||(0 == strcmp(currSensorName,SENSOR_DRVNAME_HI545QT_MIPI_RAW)))){
			...
			if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMRST])
        		...
        		else if (pinSetIdx == 1 && currSensorName && ((0 == strcmp(currSensorName, SENSOR_DRVNAME_OV2680_MIPI_RAW))||(0 == strcmp(SENSOR_DRVNAME_OV2680KD_MIPI_RAW,currSensorName)))) {
			/* First Power Pin low and Reset Pin Low */
			if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMPDN])
				mtkcam_gpio_set(pinSetIdx, CAMPDN, pinSet[pinSetIdx][IDX_PS_CMPDN + IDX_PS_ON]);
			/* VCAM_IO */
