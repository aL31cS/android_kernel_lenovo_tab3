...
#if 1 //def MTK_MULTI_BAT_PROFILE_SUPPORT
extern int IMM_GetOneChannelValue_Cali(int Channel, int *voltage);
unsigned int g_fg_battery_id = 0;

#ifdef MTK_GET_BATTERY_ID_BY_AUXADC
#define AUXADC_BATTERY_ID_CHANNEL 12
...
void fgauge_get_profile_id(void)
{
	...
	bm_print(BM_LOG_CRTI, "[fgauge_get_profile_id][1=LGC][0=COS]Battery id (%d)\n", g_fg_battery_id);		
}
#else
void fgauge_get_profile_id(void)
{
				...
				const char *node_srting, BATTERY_PROFILE_STRUCT_P profile_p)
{
	...
	if(g_fg_battery_id == 1){
		...
		msleep(20);
		bm_print(BM_LOG_CRTI, "__batt_meter_parse_table>> %s[%d]: <%d, %d>\n",
				node_srting, ((idx-152)/2), profile_p->percentage, profile_p->voltage);
		#endif

			profile_p++;
			idx++;
		if ((idx-152) >= (saddles * 2))
			break;
		}
