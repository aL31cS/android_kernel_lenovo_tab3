...
int audience_irq = 0, audience_irq1 = 0;
#endif
...
static struct esxxx_platform_data *es705_populate_dt_pdata(struct device *dev)
{
	...
	if (audience_node)
	{
		...
		ret = of_property_read_u32(audience_node, "audience_gpioa_pin", &ints1);
		if (ret)
			printk("STT: audience_gpioa_pin is not OK!\n");

		ret = of_property_read_u32(audience_node, "audience_gpiob_pin", &ints2);
		if (ret)
...
*/
	...
	if (pdata->gpiob_gpio != -1) {
#if 1 //added by shentaotao
		audience_irq1 = gpio_to_irq(pdata->gpiob_gpio);
		printk("STT: audience_irq=%d, audience_irq1=%d\n", audience_irq, audience_irq1);
