...
int escore_uart_boot_sequence(struct escore_priv *escore)
{
	...
	/* SBL SYNC BYTE 0x00 */
	printk("%s(): write ESCORE_SBL_SYNC_CMD = 0x%02x\n", __func__,
		sbl_sync_cmd);
	...
	printk("%s(): firmware load sbl sync write rc=%d\n", __func__, rc);
	memset(msg, 0, 4);
	...
	/* SBL BOOT BYTE 0x01 */
	memset(msg, 0, 4);
	printk("%s(): write ESCORE_SBL_BOOT_CMD = 0x%02x\n", __func__,
		sbl_boot_cmd);
...
int escore_uart_boot_finish(struct escore_priv *escore)
{
	...
	do {
		pr_err("%s(): write ES_SYNC_CMD = 0x%08x\n",
			__func__, sync_cmd);
...
int escore_uart_bus_init(struct escore_priv *escore)
{
	...
	printk("STT: %s before platform_device_add\n", __func__);
